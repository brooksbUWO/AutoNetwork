/**
 * @file vector.h
 * @brief Lightweight STL-like vector implementation for embedded systems
 * @version 1.1.0
 * @date 2025-11-11
 *
 * Provides a minimal dynamic array container compatible with ESP32 Arduino.
 * Used internally by AutoNetwork for credential storage and parameter management.
 */
...
/**
 * @brief Lightweight dynamic array container (STL vector replacement)
 *
 * @details Implements a subset of std::vector functionality optimized for embedded systems.
 *          Supports dynamic resizing, element access, insertion, and removal.
 *
 * @tparam VectorType Type of elements stored in the vector
 *
 * @par Features
 *      - Dynamic memory management with automatic resizing
 *      - Random access via operator[]
 *      - Stack-like operations (PushBack, PopBack)
 *      - Bulk operations (Assign, Clear)
 *      - Search operations (Contains, Find)
 *
 * @par Memory Management
 *      - Initial allocation can be specified to reduce reallocations
 *      - Automatic doubling growth strategy when capacity exceeded
 *      - Manual capacity control via Reserve()
 *
 * @par Usage Example
 * @code
 * Vector<int> numbers(10);  // Pre-allocate for 10 elements
 * numbers.PushBack(42);
 * numbers.PushBack(100);
 * int val = numbers[0];     // Access first element
 * @endcode
 */
